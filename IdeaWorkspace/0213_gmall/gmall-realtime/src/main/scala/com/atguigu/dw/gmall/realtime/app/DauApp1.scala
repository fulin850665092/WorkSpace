  ...
  def main(args: Array[String]): Unit = {
    ...
    val filterUids = startLog.transform(
      rdd => {
        ...
        val uidSet = jedisClient.smembers(GmallConstant.REDIS_DAU_KEY + ":" + new SimpleDateFormat("yyyy-MM-dd").format(new Date()))
      ...
      rdd=>{
        rdd.foreachPartition(
          iter=>{
            ...
            iter.foreach(
              log=>{
                client.sadd(GmallConstant.REDIS_DAU_KEY+":"+log.logDate,log.uid)
              }
            )
            client.close()
          }
